...
touchstone script in `touchstone/script.R` with `benchmark_run_ref()`. The
...
We sample both refs in `n` blocks. This is better than completely at random,
because that can lead to biased results if compute power steadily
increases/decreases and our sampling is very structured, i.e.:
...
So we sample randomly within blocks:
